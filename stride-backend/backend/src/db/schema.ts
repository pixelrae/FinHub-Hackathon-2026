...
export const users = sqliteTable("users", {
    ...
    walletHandle: text("wallet_handle").unique(),
...
export const transactions = sqliteTable("transactions", {
    ...
    receiverUserId: text("receiver_user_id").references(() => users.id),
    payerRef: text("payer_ref"),
    reportIncluded: integer("report_included", { mode: "boolean" }).default(
        false,
    ),
...
export const paymentRequests = sqliteTable("payment_requests", {
    ...
    requesterId: text("requester_id")
        .notNull()
        .references(() => users.id), // who gets paid
    payerId: text("payer_id")
        .notNull()
        .references(() => users.id), // who is asked to pay
